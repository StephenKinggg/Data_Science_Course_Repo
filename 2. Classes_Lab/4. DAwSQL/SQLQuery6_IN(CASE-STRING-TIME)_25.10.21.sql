...
--Sonuç olarak burada 2019 üretilip 2018 yýlýnda üretilmeyen markalarýnda olduðunu görebiliyoruz.

SELECT DISTINCT model_year --Bu þekilde de kontrol etmiþ olduk.
FROM product.product
WHERE brand_id=5
...
INNER JOIN(
			...
			SELECT A.product_id  --2019 yýlý dýþýnda sipariþ verilen ürünleri bulduk. Bunu 2019 yýlýndan çýkarýnca kalan sadece 2019 yýlýnda verilen diðer yýllarda verilmeyenlerdir.
			FROM sale.order_item A, sale.orders B
			WHERE A.order_id=B.order_id
...
--Yukarýdaki BETWEEN ile yaptýðýmýz iþlemi LIKE ile de yapabiliriz.

SELECT A.product_id
FROM sale.order_item A, sale.orders B
WHERE A.order_id=B.order_id
AND B.order_date LIKE '2019%' --2019_ dersek sadece bir karakter, 2019% ise bilinmeyen sayýda karakter var demek.
...
--Add a column to the sales.staff table containing the store name of the employees.
...
SELECT email, (charindex('@',email, 1)+1)-(charindex('@', email, 1)-1)
FROM sale.customer

SELECT email, substring(email, charindex('@', email, 1)+1,
